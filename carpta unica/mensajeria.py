...
def autenticar(ipAuth, portAuth):
    ...
    try:
        ...
        bienvenida = sock.recv(1024).decode(errors="replace")
        print(bienvenida.strip())

        sock.sendall(mensaje.encode())

        respuesta = sock.recv(1024).decode(errors="replace").strip()

        if respuesta == "SI":
            nombre_completo = sock.recv(1024).decode(errors="replace").strip()
...
def receptor(port):
    ...
    while running:
        try:
            ...
            tipo = partes[0].decode(errors="replace")
            user = partes[1].decode(errors="replace")
                ...
                try:
                    if modo == "CHUNK" and len(partes) >= 7:
                        ...
                        if len(estado["chunks"]) == total:
                            contenido_final = b"".join(
                                estado["chunks"][i] for i in range(total) if i in estado["chunks"]
                            )
                        ...
                        contenido = partes[4] if modo == "RAW" and len(partes) >= 5 else b""
                        if not contenido:
                            imprimir_error_archivo(ip_emisor, user)
                            continue
...
def enviar_mensaje(sock, destino, mensaje, puerto):
    try:
        ip_destino = destino if destino == BROADCAST_ADDR else socket.gethostbyname(destino)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Uso:")
        print("python mensajeria.py port ipAuth portAuth")
        sys.exit(1)
